...
package MyMotorDriver is
   ...
   type Directions is (Forward, LeftRotate, RightRotate, Stop); --only two are implemented but many configuration are possible with mecanum wheels
   type Statemachine is (Stop, Rotate, Forward); -- States the car can be in
   ...
   protected MotorDriver is
      ...
      function GetCurrentState return Statemachine;
      ...
      procedure SetCurrentState (V : Statemachine);
